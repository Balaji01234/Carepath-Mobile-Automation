...
describe('Onboarding - Mental Health Program', async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
        it(`${testCases[i].testId} - ${testCases[i].testDescription}`, async () => {
            try {
                const userName = await getUniqueEmailByProgramRoleAndScenario("signup", "SignUp-withApproval", "TC_01", "Mental Health", "Employee","Positive");
                if (userName !== 'undefined') {
                    await writeExcelData1("onboarding", "onboardingMH", "MulipleOptionMH", `${testCases[i].testId}`, "Email", userName);
