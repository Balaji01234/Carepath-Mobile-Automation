...
describe('Onboarding - Elder Care Program', async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
        it(`${testCases[i].testId} - ${testCases[i].testDescription}`, async () => {
            try {
                const userName = await getUniqueEmailByProgramRoleAndScenario("signup", "SignUp-withApproval", "TC_01", "Elder Care", "Employee", "Negative");
                if (userName !== 'undefined') {
                    await writeExcelData1("onboarding", "onboardingElderCare - Negative", "ElderCareNegative", `${testCases[i].testId}`, "Email", userName);
