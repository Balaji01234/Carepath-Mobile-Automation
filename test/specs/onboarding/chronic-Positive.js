...
describe('Onboarding - Chronic Program', () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
        it(`${testCases[i].testId} - ${testCases[i].testDescription}`, async () => {
            try {
                const userName = await getUniqueEmailByProgramRoleAndScenario("signup", "SignUp-withApproval", "TC_01", "Chronic Disease", "Employee", "Positive");
                if (userName !== 'undefined') {
                    await writeExcelData1("onboarding", "onboardingChronic", "chronic-positive", `${testCases[i].testId}`, "Email", userName);
