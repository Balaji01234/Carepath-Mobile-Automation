...
describe('Onboarding - Cancer Program', async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
        it(`${testCases[i].testId} - ${testCases[i].testDescription}`, async () => {
            try {
                const userName = await getUniqueEmailByProgramRoleAndScenario("signup", "SignUp-withApproval", "TC_01", "Cancer", "Employee", "Positive");
                if (userName !== 'undefined') {
                    await writeExcelData1("onboarding", "onboardingCancer", "MulipleOptionCancer", `${testCases[i].testId}`, "Email", userName);
