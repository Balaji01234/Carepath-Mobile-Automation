...
describe('Sign-Up with Approval - Positive', async () => {

    const locator = new locators();
    const Keywords = new keywords(locator);
    const timeout = process.env.DISPLAY_TIMEOUT
    let testName = '';
    const testCases = getFilteredTests("signup", "SignUp-withApproval", "TC_01");
    ...
    for (let i = 0; i < testCases.length; i++) {
        it(`${testCases[i].testId} - ${testCases[i].testDescription}`, async () => {
            try {
                const role = await readData1("signup", "SignUp-withApproval", "TC_01", "Role", `${testCases[i].testId}`);
                const description = await readData1("signup", "SignUp-withApproval", "TC_01", "Test Description", `${testCases[i].testId}`);
                allureReporter.addDescription(description)
                console.log(`Role: ${role}`)
                const employeeText = await readData1("signup", "SignUp-withApproval", "TC_01", "EmployeeText", `${testCases[i].testId}`);
                const studentText = await readData1("signup", "SignUp-withApproval", "TC_01", "StudentText", `${testCases[i].testId}`);
                const FirstName = `PrabhaAuto${getRandomString()}`
                await writeExcelData1("signup", "SignUp-withApproval", "TC_01", `${testCases[i].testId}`, "Firstname", FirstName);
                const LastName = (`Automation${getRandomString()}`).toLowerCase();
                await writeExcelData1("signup", "SignUp-withApproval", "TC_01", `${testCases[i].testId}`, "Lastname", LastName);
                const mailPrefix = await readData1("signup", "SignUp-withApproval", "TC_01", "MailPrefix", `${testCases[i].testId}`);
                const mail = `${mailPrefix}${getRandomString() + generateRandomNumber()}@mailinator.com`;
                const phoneNumber = await readData1("signup", "SignUp-withApproval", "TC_01", "Phone number", `${testCases[i].testId}`);
                const password = await readData1("signup", "SignUp-withApproval", "TC_01", "Password", `${testCases[i].testId}`);
                const employerName = await readData1("signup", "SignUp-withApproval", "TC_01", "Employer name", `${testCases[i].testId}`);
                const referral = await readData1("signup", "SignUp-withApproval", "TC_01", "Referrals", `${testCases[i].testId}`);
                const dob = await readData1("signup", "SignUp-withApproval", "TC_01", "DOB", `${testCases[i].testId}`);
                const relation = await readData1("signup", "SignUp-withApproval", "TC_01", "Relation", `${testCases[i].testId}`);
                const referralDropdownText = await readData1("signup", "SignUp-withApproval", "TC_01", "ReferralText", `${testCases[i].testId}`);
                const dobText = await readData1("signup", "SignUp-withApproval", "TC_01", "dobText", `${testCases[i].testId}`);
                const studentId = await readData1("signup", "SignUp-withApproval", "TC_01", "StudentId", `${testCases[i].testId}`);
                const courseEnrolled = await readData1("signup", "SignUp-withApproval", "TC_01", "CourseEnrolled", `${testCases[i].testId}`);
                const program = await readData1("signup", "SignUp-withApproval", "TC_01", "Program", `${testCases[i].testId}`);
                ...
                await Keywords.programApprove(mail,"approve");
                ...
                await Keywords.verifyElementDisplayed(locator.welcomeToOnboarding, "Welcome to Onboarding")
                await writeExcelData1("signup", "SignUp-withApproval", "TC_01", `${testCases[i].testId}`, "Email", mail);
                await writeExcelData1("signup", "SignUp-withApproval", "TC_01", `${testCases[i].testId}`, "Approve Status", "Approved");
            } catch (err) {
                await writeExcelData1("signup", "SignUp-withApproval", "TC_01", `${testCases[i].testId}`, "Approve Status", "Not Approved");
                throw new Error(err);
            }
        })
    }
